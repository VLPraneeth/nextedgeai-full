...
public class DateTimeUtil {
    ...
    private static ZonedDateTime convert(String value) {
        ...
        try {
            ...
            if (parsed != null) {
                return ZonedDateTime.ofInstant(Instant.ofEpochMilli(parsed.getMillis()), parsed.getZone().toTimeZone().toZoneId());
            }
    ...
    static Map<Class<?>, Function<Object, ZonedDateTime>> getConverters() {
        return CONVERTERS;
    }

    public static ZonedDateTime convert(Object value) {
        if (null == value) return null;
        for (Map.Entry<Class<?>, Function<Object, ZonedDateTime>> converter : getConverters().entrySet()) {
            ...
            if (List.class.isAssignableFrom(value.getClass())) {
                ...
                if(!valueList.isEmpty() && converter.getKey().isAssignableFrom(valueList.get(0).getClass())) {
                    return (ZonedDateTime) valueList.stream().map(val -> converter.getValue().apply(val)).collect(Collectors.toList());
                }
            }
        }
        log.debug("No converters found for value {}. Forcing string based conversion", value);
        Function<Object, ZonedDateTime> stringConverter = getConverters().get(String.class);
