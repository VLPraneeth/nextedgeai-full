...
@Ignore
public class FieldPipelineTransactionTest extends AbstractSyncariTest {
    ...
    public void lastTransactionTest() {
        ...
        when(connectorService.get(srcEntityDef.getConnectorId())).thenReturn(zendeskConnector);

        //EntityRepo entityRepo = mock(EntityRepo.class);
        executeFieldPipeline.repoService = repoService;
        ...
        doNothing().when(entityRepo).updateLastTransaction(any(), any());
        ...
        SimulationCurrentBatch currentBatch = getCurrentBatch(SerializationUtils.clone(entityData), srcEntityDef, coreEntityDef, syncariId);
        currentBatch.setExistingRecords(List.of(entityData));

        currentContext.setCurrentBatch(currentBatch);

        // Not using capture arguments, as want to capture entity list during the call
        List<EntityData> entitiesBatch = new ArrayList<>();
        doAnswer(invocation -> {
            entitiesBatch.addAll((List) invocation.getArguments()[1]);
            return null;
        }).when(entityRepo).updateLastTransaction(any(), any());
        ...
        assertEquals(entitiesBatch.get(0).getLastTransactionLogId(), transactions.get(0).getId());
    }

    private SimulationCurrentBatch getCurrentBatch(EntityData entityData, EntityDefinition srcEntity, EntityDefinition syncariEntity, String syncariId) {

        entityData.setDeleted(false);
