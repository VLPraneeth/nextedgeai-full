...
public class EnrichPersonFunctionTest extends AbstractSyncariTest {
    ...
    public void validate_clearbitCreds() {
        ...
        try{
            ...
            assertEquals("Missing Enrichment Source from Enrich Person Function in graph attribGraph", e.getMessage());
        }
        ...
        try{
            ...
            assertEquals("Invalid Enrichment Source 'serviceId123' in node Enrich Person Function of graph attribGraph", e.getMessage());
        }
        ...
        try{
            ...
            assertEquals("Invalid Enrich Using 'invalid_field' in node Enrich Person Function of graph attribGraph", e.getMessage());
        }
        ...
        try{
            ...
            assertEquals("Invalid Source Entity 'externalEntityId' in node Enrich Person Function of graph attribGraph", e.getMessage());
        }
        ...
        try{
            ...
            assertEquals("Invalid Input Field 'inputField123' in node Enrich Person Function of graph attribGraph", e.getMessage());
        }
        ...
        try{
            ...
            assertEquals("Invalid Enrichment Field 'enrichmentField123' in node Enrich Person Function of graph attribGraph", e.getMessage());
        }
        ...
        call.setConfig(Map.of("serviceId", clearbitCreds.getId(),
                ...
                "entityDefinition", externalEntity.getId(),
                "emailField", attribute.getId(),
                "lookUpKey", "person.name.fullName"));
        funcConfig.setFunctionCall(call);
        node = nodeRepo.save(node.setConfiguration(funcConfig));
        function.validate(context);
    ...
    public void validateDestinationSideFunction(){
        ...
        MappingGraph graph = GraphHelper.newGraph(syncariAttrib, functionService)
                ...
                .function("enrichPerson", "enrichPerson", badEnrichConfig)
                .dest(attribute, "destAttribute")
                .connect("srcAttribute", syncariAttrib.getApiName())
                .connect(syncariAttrib.getApiName(), "enrichPerson")
                .connect("enrichPerson", "destAttribute").getGraph();

        MappingNode enrichPersonNode = graph.getNodes().stream().filter(n -> n.getName().equals("enrichPerson")).findFirst().get();
        ValidationContext context = new ValidationContext().setGraph(graph).setNode(enrichPersonNode)
                .setSyncariConnector(connectorService.getSyncariConnector()).setCoreEntity(syncariEntity)
                .setSourceEntityMap(Map.of(externalEntity.getId(), externalEntity));
        ...
        try{
            ...
            assertEquals("Invalid Source Entity '"+externalEntity.getId()+"' in node enrichPerson of graph AboutUs", e.getMessage());
        }

        SimpleFunctionNodeConfig functionNodeConfig = enrichPersonNode.getTypedConfiguration();
        Map<String, Object> enrichConfig = Map.of("serviceId", clearbitCreds.getId(),
                "enrichUsing", "email",
                "entityDefinition", syncariEntity.getId(),
                "emailField", syncariEntity.getFieldByName("Name").getId(),
                "lookUpKey", "person.name.fullName");
    ...
    public void validateSourceSideFunction(){
        ...
        MappingGraph graph = GraphHelper.newGraph(syncariAttrib, functionService)
                .src(attribute, "srcAttribute")
                .function("enrichPerson", "enrichPerson", enrichConfig)
                .connect("enrichPerson", "srcAttribute")
                .connect("enrichPerson", syncariAttrib.getApiName()).getGraph();
        ...
        graph = GraphHelper.newGraph(syncariAttrib, functionService)
                .src(attribute, "srcAttribute")
                .function("enrichPerson", "enrichPerson", enrichConfig)
                .connect("srcAttribute", "enrichPerson")
                .connect("enrichPerson", syncariAttrib.getApiName()).getGraph();
        enrichPersonNode = graph.getNodes().stream().filter(n -> n.getName().equals("enrichPerson")).findFirst().get();
        functionNodeConfig = enrichPersonNode.getTypedConfiguration();
        functionNodeConfig.getFunctionCall().setConfig(enrichConfig);
        context = new ValidationContext().setGraph(graph).setNode(enrichPersonNode)
                .setSyncariConnector(connectorService.getSyncariConnector()).setCoreEntity(syncariEntity)
                .setSourceEntityMap(Map.of(externalEntity.getId(), externalEntity));
