...
@Component(ActionConstants.CREATE_EXTERNAL_RECORD)
public class CreateExternalRecordAction extends DefaultAction {
    ...
    public List<ValidationError> validateWithoutException(ValidationContext validationContext) {
        ...
        if (entityId != null) {
			...
			validateCondition(ValidationError.scopedError(node.getScope(), node.getId()),
					...
					i18n("invalid_config_in_node", configNameLabelMap.get("insertFields"), "Empty Fields", node.getName(), graph.getName()),
    ...
    public MappingNode resolve(QuickStartContext context) {
        ...
        // 3. attributes from updateFields config
        List<Map<String, Map<String, String>>> updateFields = (List<Map<String, Map<String, String>>>) actionNodeConfig.getConfigMap().get("insertFields");
        for (Map<String, Map<String, String>> s : updateFields) {
        ...
        resolvedConfigMap.put("insertFields", updateFields);
    ...
    public List<Pair<String, String>> toUserFriendlyValue(DiffInfoContext context, String configProperty) {
    	...
    	} else if ("insertFields".equals(configProperty)) {
    		Map<String, Object> configMap = context.getCurrentNode().getConfiguration().getConfigMap();
