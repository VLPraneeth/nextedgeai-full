...
public class LineVizConfig extends VizConfig {
    QueryField xAxis;
    List<QueryField> yAxis;
    List<QueryField> series;
    ...
    public VizConfig makeCopy(){
        LineVizConfig func = new LineVizConfig();
        ...
        if (CollectionUtils.isNotEmpty(this.getYAxis())){
            ...
            ((LineVizConfig)func).setYAxis(cols);
        }
        if (CollectionUtils.isNotEmpty(this.getSeries())){
            ...
            ((LineVizConfig)func).setSeries(cols);
        }
        if (null != xAxis){
            ((LineVizConfig)func).setXAxis(xAxis.makeCopy());
        }
        ((LineVizConfig)func).setStacking(stacking);
