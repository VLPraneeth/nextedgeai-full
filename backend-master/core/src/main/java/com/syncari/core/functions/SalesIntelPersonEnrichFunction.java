...
@Slf4j
@Component(SalesIntelFunctionsSeed.SALESINTEL_PERSON_ENRICH)
public class SalesIntelPersonEnrichFunction extends DefaultFunction{
	...
	public List<ValidationError> validateWithoutException(ValidationContext validationContext) {
		...
		String email = functionCall.getConfig().getOrDefault("email", "").toString();
		String phoneNumber = functionCall.getConfig().getOrDefault("phoneNumber", "").toString();
		String connectorId = functionCall.getConfig().getOrDefault("serviceId", "").toString();

		boolean hastTokensEmail = tokenHelper.hasTokens(email);
		boolean hastTokensPhoneNumber = tokenHelper.hasTokens(phoneNumber);
		validateCondition(ValidationError.scopedError(node.getScope(), node.getId()),
				hastTokensEmail || hastTokensPhoneNumber, "si-validation-invalid-input", email, node.getName(),
				graph.getName(), ErrorCode.E1124.getCode()).ifPresent(e -> errors.add(e));
		...
		validateCondition(ValidationError.scopedError(node.getScope(), node.getId()),
				...
				node.getName(), graph.getName()), ErrorCode.E1125.getCode()).ifPresent(e -> errors.add(e));
		connector.ifPresent(c -> {
			try {
				...
				validateCondition(ValidationError.scopedError(node.getScope(), node.getId()),
						...
						node.getName(), graph.getName()), ErrorCode.E1126.getCode()).ifPresent(e -> errors.add(e));
			} catch (Exception e) {
				log.error("validation error occured ", e);
				validateCondition(ValidationError.scopedError(node.getScope(), node.getId()), true,
						i18n("invalid_salesintel_credentials_create", c.getName(), node.getName(), graph.getName()), ErrorCode.E1127.getCode())
    ...
    public void extract(QuickStartContext context) {
        ...
        DependencyUtil.getTokenDependencies(email).forEach(d -> qsConfig.addDependency(d));
        DependencyUtil.getTokenDependencies(phoneNumber).forEach(d -> qsConfig.addDependency(d));
    }
    ...
    public MappingNode resolve(QuickStartContext context) {
        ...
        String email = configMap.getOrDefault("email", "").toString();
        String resolvedEmail = (String) qsConfig.getResolvedValueByType(email, QSDependency.Type.Token);
        if(resolvedEmail != null){
            configMap.put("email", resolvedEmail);
        }
        String phoneNumber = configMap.getOrDefault("phoneNumber", "").toString();
        String resolvedPhoneNumber = (String) qsConfig.getResolvedValueByType(phoneNumber, QSDependency.Type.Token);
        if(resolvedPhoneNumber != null){
            configMap.put("phoneNumber", resolvedPhoneNumber);
        }
