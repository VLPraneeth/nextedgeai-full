...
@Slf4j
@Component(FunctionConstants.ENRICH_COMPANY)
public class EnrichCompanyFunction extends DefaultFunction {
    ...
    @Autowired
    ServiceCredentialService serviceCredService;
    ...
    private static final String INPUT_FIELD_ID = "domainField";
    ...
    public List<ValidationError> validateWithoutException(ValidationContext validationContext) {
		...
		validateCondition(ValidationError.scopedError(node.getScope(), node.getId()),
				serviceCred.isEmpty() && connector.isEmpty(), i18n("invalid_config_in_node",
						configNameLabelMap.get(SERVICE_ID), serviceId, node.getName(), graph.getName()), ErrorCode.E1094.getCode())
        ...
        if(serviceCred.isPresent()){
            inputFields = clearbitService.getInputFields("company");
            outputFields = clearbitService.getOutputFields("company");
            ...
            inputFields = service.getInputFields(connectorInfo, "company");
            outputFields = service.getOutputFields(connectorInfo, "company");
        }
		...
		validateCondition(ValidationError.scopedError(node.getScope(), node.getId()),
				!inputFields.keySet().contains(enrichUsing), i18n("invalid_config_in_node",
						configNameLabelMap.get(ENRICH_USING), enrichUsing, node.getName(), graph.getName()), ErrorCode.E1095.getCode())
								.ifPresent(e -> errors.add(e));
		...
		validateCondition(ValidationError.scopedError(node.getScope(), node.getId()),
				...
				i18n("invalid_config_in_node", configNameLabelMap.get(SOURCE_ENTITY_ID), sourceEntityId, node.getName(),
						graph.getName()), ErrorCode.E1096.getCode()).ifPresent(e -> errors.add(e));
		...
		validateCondition(ValidationError.scopedError(node.getScope(), node.getId()),
				...
				i18n("invalid_config_in_node", configNameLabelMap.get(SOURCE_ENTITY_ID), sourceEntityId, node.getName(),
						graph.getName()), ErrorCode.E1097.getCode()).ifPresent(e -> errors.add(e));
        ...
        try {
        	...
        	validateCondition(ValidationError.scopedError(node.getScope(), node.getId()), !isFieldFromSourceEntity,
        			i18n("invalid_config_in_node", configNameLabelMap.get(INPUT_FIELD_ID), inputFieldId, node.getName(),
        					graph.getName()), ErrorCode.E1098.getCode()).ifPresent(e -> errors.add(e));
		...
		validateCondition(ValidationError.scopedError(node.getScope(), node.getId()),
				!outputFields.keySet().contains(lookupFieldName), i18n("invalid_config_in_node",
						configNameLabelMap.get(LOOKUP_FIELD_NAME), lookupFieldName, node.getName(), graph.getName()), ErrorCode.E1099.getCode())
								.ifPresent(e -> errors.add(e));
