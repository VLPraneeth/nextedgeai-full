...
@Slf4j
public class SYN_8458_DeleteDatastoreEntity {

    @ChangeSet(order = "001", id = "deleteDatastoreEntity", author = "abhinav", runAlways = true)
    public void deleteDatastoreEntity(MongoTemplate db) {
        ...
        if(!dryRunMode) {
            datastoreService.truncateEntity(syncariEntity, datastore);
        }
    }
}
