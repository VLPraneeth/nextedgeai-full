...
@Slf4j
public class SYN_8610_BOMCheckImportFiles {
	
	private static final String BOM_CHAR = "\uFEFF";

	@ChangeSet(order = "001", id = "bomCheckImportFiles", author = "sibin")
	public void bomCheckImportFiles(MongoTemplate db) {
		...
		if (importFilesConnector != null) {
			...
			while (attribs.hasNext()) {
				...
				if (displayName != null && (displayName.startsWith(BOM_CHAR))) {
					log.info("Attribute with BOM character {} ", displayName);
				}
			}
		}
