...
import { ConditionValue, FilterValue, isConditionValue } from 'components/inputs/types';
import AppConstants from 'utils/AppConstants';
import { tNamespaced } from 'utils/i18nUtil';

import FilterCondition, { FilterConditionValue } from './DataStudioFilterCondition/FilterCondition';

import './index.less';
import {
  FilterProps as CoreFilterProps,
  FilterRef,
  InternalFilterProps as CoreInternalFilterProps,
} from 'components/inputs/filter';
...
// Data Studio specific filter props that extend the core filter props
interface DataStudioFilterProps extends CoreFilterProps {
  hideLeftInput?: boolean;
  verticalLayout?: boolean;
  isAllDisabled?: boolean;
  placeHolder?: string;
}
interface DataStudioInternalFilterProps extends CoreInternalFilterProps {
  hideLeftInput?: boolean;
  verticalLayout?: boolean;
  isAllDisabled?: boolean;
  placeHolder?: string;
...
const DataStudioFilter = forwardRef<FilterRef, DataStudioFilterProps | DataStudioInternalFilterProps>(
  (
    {
      name,
      displayMode,
    ...
    const initializePredicates = (
      val: DataStudioFilterProps['value'],
      defaultVal: DataStudioFilterProps['defaultValue']
    ) => {
      let initialValue;
    ...
    const _getGroupOperator = () => {
      ...
      return (
        <div className="filter-operator-container">
          <Dropdown overlay={menu} trigger={['click']} disabled={isAllDisabled}>
      ...
      return (
        <div className={cls} key={`filter-condition-container-${predicate?.predicateId}`}>
          <FilterCondition
            ...
            hideLeftInput={hideLeftInput}
            verticalLayout={verticalLayout}
            allFieldsDisabled={isAllDisabled}
            placeHolder={placeHolder}
          />
        </div>
      );
...
export default DataStudioFilter;
