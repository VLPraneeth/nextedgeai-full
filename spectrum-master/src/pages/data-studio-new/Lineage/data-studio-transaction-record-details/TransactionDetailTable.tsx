...
const TransactionDetailTable = ({ transaction }: TransactionDetailTableProps) => {
    ...
    return [
      {
        ...
        tooltipValueGetter: (params: ITooltipParams) => {
          const data = params.data as TransactionChange;
          const displayName = data?.displayName || data?.apiName || '';
          const apiName = data?.apiName || '';
          return displayName && apiName ? `${displayName} [${apiName}]` : displayName || apiName;
        },
      },
      {
        autoHeight: true,
  ...
  return (
    <Stack className="synri-transaction-detail-table-container" fill scrollOverflow>
      <HStack spacing="lg">
        <Stack spacing="xxxs">
          <TranslatedText namespace="Common" text="created_at" color="gray-700" />
          <div className="header-link">
            <TransactionDate text={transaction.createdAt} />
          </div>
        </Stack>
        <Stack spacing="xxxs">
          <TranslatedText namespace="Common" text="syncari_id" color="gray-700" />
