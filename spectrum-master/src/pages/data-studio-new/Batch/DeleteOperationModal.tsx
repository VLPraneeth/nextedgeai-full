...
const DeleteModal = ({ commonI18nArgs: i18nArgs, mode, entity, fieldValues, filter }: DeleteOperationModalProps) => {
  ...
  const okButtonTitle = useMemo(() => {
    if (mode === BatchOperationMode.PURGE) {
      return tn('purge_records_btn_with_count', i18nArgs);
    }
    if (mode === BatchOperationMode.DELETE) {
      return tn('delete_records_btn_with_count', i18nArgs);
    }
  ...
  const modalTitle = tn('delete_modal_title');
  ...
  return (
    <EnhancedModal
      ...
      <Stack>
        ...
        <Stack spacing="lg">
          {
            // if we're in DELETE mode, show scope
                ...
                {filter ? (
                  [
                    <TranslatedText
                      key="label"
                      beDangerous
                      text="delete_records_warning_text_plural"
                      args={i18nArgs}
                    />,
          ...
          <Stack spacing="sm" className="delete-modal-content">
            {!filter && <Text beDangerous>{deleteWarningText}</Text>}
            ...
            {mode === BatchOperationMode.DELETE && (
              <Checkbox
                ...
                {tn('global_delete', { count: i18nArgs?.count })}
              </Checkbox>
            )}
          </Stack>
        </Stack>
