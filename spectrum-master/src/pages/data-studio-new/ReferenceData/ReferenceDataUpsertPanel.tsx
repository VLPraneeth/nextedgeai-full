...
const ReferenceDataPanel = ({ onRequestClose, referenceData, visible }: ReferenceDataModalProps) => {
  const { tn } = useI18nContext();
  const dispatch = useEnhancedDispatch();
  const stateReferenceData = useEnhancedSelector((state) => state.referenceData);
  const error = stateReferenceData.upsertError[referenceData?.id || 'new'];
  const status = stateReferenceData.upsertStatus[referenceData?.id || 'new'];
  const { refetch: refetchReferenceDataList } = useReferenceDataList();
  ...
  // Clear error from Redux when modal closes
  useEffect(() => {
    if (!visible) {
      const id = referenceData?.id || 'new';
      dispatch(clearReferenceDataUpsertError(id));
    }
  }, [visible, referenceData?.id, dispatch]);
  ...
  const save = async () => {
    if (validateInputs()) {
      ...
      // Only refetch preview and show success if the upsert succeeded
      if (result.meta.requestStatus === 'fulfilled') {
        // Get the reference data ID from the result payload
        const refDataId = (result.payload as ReferenceDataRecord)?.id;

        // Fetch preview for the newly created/updated reference data and refetch the list
        // to ensure counts are synced in grid, dropdown, and pagination
        if (refDataId) {
          await Promise.all([dispatch(getReferenceDataPreview(refDataId)), refetchReferenceDataList()]);
        }
  ...
  return (
    <DrawerPanel
      ...
      onClose={closePanel}
      data-testid="mock-reference-upsert-modal"
      absolutePositioning
      width="xlarge"
      destroyOnClose
      className="reference-data-upsert-modal"
      mask
      maskClosable
      visible={visible}
      footer={
...
export default withI18n(ReferenceDataPanel, 'ReferenceDataModal');
